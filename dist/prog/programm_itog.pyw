...
try:
    with open(f'user_{per}.txt', mode='r+', encoding='utf-8') as file_cash:
    ...
    with open(f'user_{per}.txt', mode='w', encoding='utf-8') as file_cash:
        file_cash.write(f'{func.verif_date(date_inf)} -- {func.verif_date(hour_inf)}:'
                        f'{func.verif_date(minut_inf)}:'
                        f'{func.verif_date(sec_inf)}\n')
        last_seen = ''
...
class Ui_MainWindow(object):
    ...
    def del_note(self):
        os.system('del_win.pyw')

    def history(self):
        os.system('output_win.pyw')
